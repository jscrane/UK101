...
ps2kbd::ps2kbd (): Memory::Device(1024), _last(0), _delay(128) {
  for (int i=8; i--; )
    _rows[i] = 0;
  // shift-lock on
  _set(0x00, true);
...
bool ps2kbd::_map (unsigned key, unsigned short &map) {
  switch (key) {
    case 0x08: 
      map = 0x62;
      break;
...
void ps2kbd::_reset (byte k) {
	byte &r = _rows[(k & 0xf0) >> 4];
	byte c = 1 << (k & 0x0f);
	r &= ~c;
}

void ps2kbd::up (unsigned key) {
	unsigned short k;
...
void ps2kbd::_set (byte k, bool lock) {
	byte &r = _rows[(k & 0xf0) >> 4];
	byte c = 1 << (k & 0x0f);
	if (lock)
		r ^= c;
